...
Realtime Dashboards in Fabric are not publicly available yet. However, to build one, they use the same concepts as Realtime Dashboards in ADX Web UI (also known as KWE - Kusto Web Explorer). Proceed to build a realtime dashboard using [ADX Web UI](https://aka.ms/adx.try) as part of this learning excercise, or you may skip this challenge considering the feature is not yet in Microsoft Fabric. Leverage the outputs of the 3 queries below (using ingestionLogs table) connected to your Fabric KQL DB uri. 
